...
class AppPaddings extends EdgeInsets {
  ...
  factory AppPaddings.all(double? value) => AppPaddings.all(value ?? 0);
  factory AppPaddings.only({double? left, double? top, double? right, double? bottom}) => AppPaddings.only(left: left ?? 0, top: top ?? 0, right: right ?? 0, bottom: bottom ?? 0);
  factory AppPaddings.fromLTRB(double? left, double? top, double? right, double? bottom) => AppPaddings.fromLTRB(left ?? 0, top ?? 0, right ?? 0, bottom ?? 0);
  factory AppPaddings.symmetric({double? horizontal, double? vertical}) => AppPaddings.symmetric(horizontal: horizontal ?? 0, vertical: vertical ?? 0);
  ...
  static EdgeInsets get updateButtons => const EdgeInsets.fromLTRB(50, 0, 50, 50);
}
